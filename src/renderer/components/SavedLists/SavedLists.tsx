...
import { getCurrentList, getLists, loadModsFromList } from '../../redux/slices/listManager';

const SingleSavedList = ({ list, index }: { list: SaveList; index: number }) => {
        ...
        <Box sx={{ border: 'solid 1px gray', borderRadius: '0.2rem', pb: 1, overflowY: 'auto' }} height={600}>
            ...
            <Stack spacing={1} divider={<Divider flexItem />}>
                {Object.keys(lists).map((key, index) => (
                    <SingleSavedList list={lists[key]} key={index} index={index} />
                ))}
            </Stack>
        </Box>
    );
